...
//            }
    ...
    @Override
    public void onSignOff(String id) {
        StatisticalManage.get().onProfileSignOff(id);
    }
    ...
    private void doNetworkStatus(Boolean networkStatus) {
        if (mOnNetworkErrorRefreshClickListener == null) {
            return;
        }
    ...
    public DialogOption showFeedbackDialog(long userId, String email) {
        ...
        if (ivCloseBtn != null) {
            ivCloseBtn.setOnClickListener(v -> op.cancel());
        }
        if (cwvWeb != null) {
     ...
     * 显示积分兑换对话框
     */
    public DialogOption showIntegralPointsRedeemDialog(int integralCount, Consumer<Integer> callOk) {
        DialogOption op = Dialog
                .with(this)
        ...
        TextView tvInsufficientPoints = op.findViewById(R.id.dialog_integral_points_redeem_tv_insufficient_points);
        ...
        if (etLightningNum != null) {
            etLightningNum.addTextChangedListener(new TextWatcher() {
                ...
                public void onTextChanged(CharSequence s, int start, int before, int count) {
                    ...
                    //更新兑换状态（兑换按钮，兑换不足）
                    changedIntegralPointsRedeemStatus(val > integralCount, tvInsufficientPoints, tvNowBtn);
                    if (val > integralCount) {
                        return;
                    }
                    //更新兑换的积分数量
                    if (tvIntegralNum != null && ObjUtils.parseInt(tvIntegralNum.getText().toString()) != val) {
    ...
    public void showCommentsKeyboard(@Nullable String defHintText, Consumer<String> call) {
        ...
        if (sivUserPhoto != null) {
            sivUserPhoto.setVisibility(View.GONE);
        }
        if (tvCommentClick != null) {
            tvCommentClick.setVisibility(View.GONE);
        }
        if (etComments != null) {
            //默认的Hint文本
            if (!TextUtils.isEmpty(defHintText)) {
                etComments.setHint(defHintText);
            }
    ...
    public void hideNetworkErrorView() {
        if (nevNetworkError != null && nevNetworkError.isShowing()) {
            nevNetworkError.hide();
        }
    }
    ...
    public int onStatusBarIconColor() {
        return SysUtil.StatusBarIconColor.BLACK;
    }

    public void dismissDialog(@Nullable DialogOption op) {
        if (op != null) {
            op.dismiss();
        }
    }
...
//            tvContent.setMovementMethod( ScrollingMovementMethod.getInstance() );
    ...
    public void showUnlockVipDialog(String uid,
                                    ...
                                    String title,
                                    String content) {
        showUnlockVipDialog(uid, name, typeId, lessonId, lessonName, imgUrl, title, content, null);
    }
    ...
    public DialogOption showEditUserNameDialog(CharSequence oldName, Consumer<String> l) {
        ...
        if (llLayout != null) {
            ...
            tvConfirm.setOnClickListener(v -> {
                ...
                if (isClick) {
                    if (isEq) {
                        l.accept(etName.getText().toString());
                    }
                    op.dismiss();
                }
            });
...
//            }
    ...
    private DialogOption getTipsDialog(String title, View.OnClickListener rightBtnListener) {
        DialogOption op = getTipsDialog(
                title,
                R.string.stringNo,
                R.string.stringYes,
...
//        if( ivBg != null ) ivBg.setImageResource( bg );
        if (tvTitle != null) {
            if (!isNullOfTitle) {
                tvTitle.setText(ResUtil.fromHtmlOfResImg(this, title, R.drawable.class));
            }
            if (titleColor != 0) {

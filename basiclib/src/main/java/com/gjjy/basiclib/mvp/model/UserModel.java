...
public class UserModel extends MvpModel implements DOM.OnResultListener {
    ...
    public RemindModel getRemindModel() { return mRemindModel; }

    public long getUserId() { return mData.getUserId(); }

    public String getUid() { return mReqUser.getUid(); }

    public String getEmail() { return mData.getEmail(); }

    public String getToken() { return mReqUser.getToken(); }

    public String getUserName(Resources res) {
        if( isLoginResult() ) return mData.getNickname();
        if( res == null ) return "Guest";
        return res.getString( R.string.stringSignature );
    }

    public String getAvatarUrl() { return mData.getAvatarUrl(); }

    public int getVipStatus() { return mData.getVipStatus(); }
    public void setVipStatus(int vipStatus) { mData.setVipStatus( vipStatus ); }

    public boolean isVip() { return mData.getIsVip(); }
    ...
    public int getIntegral() { return mData.getIntegral(); }

    public String getInvitationCode() { return mData.getInvitationCode(); }

    @LoginType
    public String getLoginType() { return mData.getLoginType(); }

    public boolean isEmailLogin() {
        return LoginType.EMAIL.equals( getLoginType() );
    }

    public boolean isExistUid() { return !TextUtils.isEmpty( mReqUser.getUid() ); }

    public boolean isLoginResult() {
        return isExistUid() && !TextUtils.isEmpty( mReqUser.getToken() );
    }

    public boolean isDeleteUser() { return mData.getIsDelete(); }

    public boolean isAddAnswer() { return mData.getIsAddAnswer(); }

    public int getHeart() { return mHeart; }

    public int getLightning() { return mLightning; }

    public int getExpCount() { return mData.getExpCount(); }

    public String getFriendInvitationCode() { return mData.getFriendInvitationCode(); }
    ...
    public void doLogOutOfOnActivityResult(Context context, int requestCode, int resultCode) {
        if( requestCode != StartUtil.REQUEST_CODE_EXIT_LOGIN_RESULT ) return;
...
//        }
    ...
    public UserDetailEntity getUserDetail() { return mData; }
...
//                if( !TextUtils.isEmpty( data.getBindType() ) &&
//                        !TextUtils.isEmpty( data.getBindUid() ) ) {
//                    mData.setToken( "--" );
//                }
            }
    ...
    public void editUserInfo(@Nullable String loginId,
        ...
        mReqUser.reqEditUserInfo(nickname, imgUrl, gender, phone, email, result -> {
            if( result ) {
                if( !TextUtils.isEmpty( loginId ) ) mData.setLoginId( loginId );
                if( !TextUtils.isEmpty( nickname ) ) mData.setNickname( nickname );
                if( !TextUtils.isEmpty( showUrl ) ) mData.setAvatarUrl( showUrl );
                if( gender != -1 ) mData.setGender( gender );
                if( !TextUtils.isEmpty( phone ) ) mData.setPhone( phone );
                if( !TextUtils.isEmpty( email ) ) mData.setEmail( email );
                mUserSQL.updatedUserDetail( mData );
            }
            if( call != null ) call.accept( result );
        });
    ...
    private void bind(String uid, String token, String loginType, Consumer<Integer> call) {
        Consumer<String> bindCall = msg -> {
            updateIdentity( uid, token, loginType );
            //绑定推送
            bindPush( r2 -> {
                if( call != null ) call.accept( r2 ? 1 : 0 );
    ...
    public void loginUid(String bindType, String bindUid, Consumer<Integer> call) {
        mReqUser.reqLoginUid(bindType, bindUid, data -> {
            ...
            if( !TextUtils.isEmpty( uid ) ) {
                bind( uid, token, loginType, call );
            }else {
                //尚未绑定账号
                bindAccount(bindType, bindUid, result -> {
                    if( call != null ) call.accept( 2 );
                });
            }
        });
    }
    ...
    public void loginEmail(String email, String password, Consumer<Integer> call) {
        mReqUser.reqEmailLoginUid(email, password, data -> {
            ...
            if( !data.isSuccess() || TextUtils.isEmpty( token ) ) {
                //通过errorNumber判断具体结果
                if( call != null ) call.accept( data.getErrorNumber() );
                return;
            }
            boolean isExist = !TextUtils.isEmpty( uid );
            //uid存在时表示绑定过账号，此时更新一次uid
            if( isExist ) {
                bind( uid, token, LoginType.EMAIL, result -> {
                    if( call != null ) call.accept( result );
                } );
                return;
            }
    ...
    private void updateIdentity(String uid, String token, @LoginType String type) {
        mReqUser.setUid( uid, token );
        mData.setUid( uid );
        mData.setToken( token );
        mData.setLoginType( type );
        // 因为前面设置token null ,现在登录需要注册新的token，不然出现登录一直不成功的场景
        com.gjjy.pushlib.Push.get().setRegistrationID( token );
        mUserSQL.updatedUserDetail( mData );
    }
    ...
    private void bind(int result, Consumer<Integer> call) {
        //保存绑定结果
        if( !mData.getIsBindAccount() ) mData.setIsBindAccount( result == 1 );
        mUserSQL.updatedUserDetail( mData );
        if( call != null ) call.accept( result );
    }
...
//        if( isBindPush ) return;
        Push.get().bindRegistrationID(token -> {
            if( TextUtils.isEmpty( token ) ) return;
            mReqUser.reqPushBind( token, r -> {
//                isBindPush = r;

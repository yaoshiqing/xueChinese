...
public class SkuUtils {
    ...
    private static boolean checkSubscriptionPeriod(String subscriptionPeriod) {
        if( subscriptionPeriod == null ) return false;
    ...
    public static int toSubscriptionPeriodOfDate(String subscriptionPeriod) {
        if( !checkSubscriptionPeriod( subscriptionPeriod ) ) return 0;
    ...
    public static char toSubscriptionPeriodOfDateSymbol(String subscriptionPeriod) {
        if( !checkSubscriptionPeriod( subscriptionPeriod ) ) return DateSymbol.NONE;
    ...
    public static int getPriceBeginIndex(String price) {
        if( TextUtils.isEmpty( price ) ) return -1;
        char[] priceArr = price.toCharArray();
        for( int i = 0; i < priceArr.length; i++ ) {
            char chr = priceArr[ i ];
            if( chr >= 48 && chr <= 57 ) return i;
        }
    ...
    public static String getDateString(Resources res, String subscriptionPeriod,
                                       boolean isCaps, boolean isPlural) {
        if( !checkSubscriptionPeriod( subscriptionPeriod ) ) return "";

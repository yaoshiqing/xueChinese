...
public class DiscoveryDetailsPresenter extends MvpPresenter<FindDetailsView> {
        ...
        mDisArtModel.reqFindDetail( mId, data -> {
            viewCall(v -> {
                ...
                if( data == null || data.getDiscoverArticleId() == 0 ) {
                    if( getContext() != null ) ToastManage.get().showToast( getContext(), R.string.stringError );
                    finish();
                    return;
                }
                //会员检查
                if( !checkIsVip( data ) ) return;
                ...
                switch( mDiscoverTypeId = data.getDiscoverTypeId() ) {
                    case 1001:      //每日聆听
                        ...
                        break;
                    case 1002:      //热门视频
                        doPopularVideosDetails( mWebUrl = data.getHtmlUrl() );
                        break;
                    case 1003:      //专项学习
    ...
    private boolean checkTargetedLearningDetailsInitGuide() {
        if( !SpIO.isShowTargetedLearningDetailsInitGuide( getContext() ) ) return false;
...
//        data.setVip( true );
    ...
    private void doListenDailyDetails(String imgUrl,
                                        List<DiscoveryDetailJsonContentSentenceEntity> sentenceList) {
        ...
        for( DiscoveryDetailJsonContentSentenceEntity sentence : sentenceList ) {
            VerticalScrollSubtitlesView.VssAdapter.ItemData subtitlesData =
                    new VerticalScrollSubtitlesView.VssAdapter.ItemData();
    ...
    /**
     ...
     @return                转换结果
     */
    private List<TargetedLearningDetailsDialogueAdapter.ItemData> toTargetedLearningDetailsTalkList(
            List<DiscoveryDetailJsonContentTalkEntity> talkList) {
        ...
        for( DiscoveryDetailJsonContentTalkEntity data : talkList ) {
            ...
            for( String zh : data.getZhSentence() ) {
                ...
                for( DiscoveryDetailJsonContentSentenceEntity key : data.getKeyword() ) {
                    ...
                    if( TextUtils.isEmpty( keyZh ) || !keyZh.equals( zh ) ) continue;
    ...
    /**
     ...
     @return                转换结果
     */
    private List<TargetedLearningDetailsGrammarAdapter.ItemData> toTargetedLearningDetailsGrammarList(
            List<DiscoveryDetailJsonContentGrammarEntity> grammarList) {
    ...
    private String toDisColorText(String text, String... disColorText) {
        if( TextUtils.isEmpty( text ) || disColorText == null || disColorText.length == 0 ) return text;
    ...
    public void updateBuriedPointTargetedLearningViewDur(int type) {
        if( mEnterTime == 0 ) mEnterTime = System.currentTimeMillis();

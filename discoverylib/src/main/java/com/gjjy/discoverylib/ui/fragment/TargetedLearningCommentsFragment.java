...
//        rgTable = findViewById( R.id.targeted_learning_details_comments_rg_table_btn );
        ...
        findViewById( R.id.comments_ll_edit_layout )
                .setBackgroundResource( R.drawable.shape_comments_send_layout_bg );
    ...
    private void initData() {
        ...
        rvList.setAdapter(
                mPresenter.isShowHotComments() ? mAdapterOfHot : mAdapterOfNew
        );
    }
    ...
    private void initListener() {
        ...
        RadioGroup.OnCheckedChangeListener onTableCheckChangeListener = (group, checkedId) -> {
            mPresenter.setShowHotComments(
                    checkedId == R.id.targeted_learning_details_comments_rb_trending_btn
            );
    ...
    @Override
    public void onCallCommentData(CommentListAdapter.CommentData data, int page,
                                  boolean isHotComments, boolean isTopComment) {
...
//        int pos = isTopComment ? 0 : -1;
        ...
        post( () ->
                ivNotData.setVisibility( adapter.getItemCount() == 0 ? View.VISIBLE : View.GONE )
        );
...
//        prlRefreshLayout.setVisibility( adapter.getItemCount() > 0 ? View.VISIBLE : View.GONE );
    ...
    @Override
    public void onCallCommentChildData(int talkId, CommentListAdapter.CommentData data,
                                       boolean isHotComments, boolean isNotify) {
        CommentListAdapter adapter = isHotComments ? mAdapterOfHot : mAdapterOfNew;
        CommentListAdapter.ItemData itemData = null;
        for( CommentListAdapter.ItemData findData : adapter.getDataList() ) {
            if( findData.getTalkId() != talkId ) continue;
            itemData = findData;
            break;
        }
//        if( itemData == null || data.getItemList().size() == 0 ) return;
        ...
        if( isNotify ) adapter.notifyDataSetChanged();
    }
    ...
    @Override
    public void onCallSendMsgResult(CommentListAdapter.ItemData data, int mainPos, int childPos,
                                    boolean result) {
        if( getActivity() == null ) return;
...
//                    }
    ...
    public void onCallLikeResult(int mainPos, int childPos, boolean result) {
        CommentListAdapter adapter = (CommentListAdapter) rvList.getAdapter();
        if( adapter == null || childPos == -1 ) return;
        ...
        if( result ) itemData.setLikeCount( itemData.getLikeCount() + 1 );
        adapter.notifyItemChanged( pos );
    }
}

...
//        fPopularVideosFragment = (PopularVideosFragment) findFragmentById( R.id.find_f_popular_videos );
        ...
        if( !isKoLang() ) ft.add( R.id.find_ll_fragment_layout, fPopularVideosFragment = new PopularVideosFragment() );
        ft.add( R.id.find_ll_fragment_layout, fTargetedLearningFragment = new TargetedLearningFragment() );
        ft.show( fListenDailyFragment );
        if( !isKoLang() ) ft.show( fPopularVideosFragment );
...
//                post(() -> doNetworkStatus( (Boolean) data ) );
                if( (Boolean) data ) {
                    if( fListenDailyFragment != null ) fListenDailyFragment.notifyUpdatedData();
                    if( !isKoLang() ) {
                        if( fPopularVideosFragment != null ) fPopularVideosFragment.notifyUpdatedData();
                    }
                    if( fTargetedLearningFragment != null ) fTargetedLearningFragment.notifyUpdatedData();
                }
                break;
        }
    }

    private boolean isKoLang() { return "ko".equals( Config.getLang() ); }
}